...
Handler = Proc.new do |req, res|
  ...
  res['Access-Control-Allow-Methods'] = 'GET, POST, PUT, DELETE, OPTIONS, PATCH'
  ...
  when 'GET /api/clients'
    ...
    res.body = DB[:clients].all.to_json
  when 'GET /api/appointments'
    res.status = 200
    res['Content-Type'] = 'application/json'
    res.body = DB[:appointments].all.to_json
  when 'GET /api/appointments/with_clients'
    res.status = 200
    res['Content-Type'] = 'application/json'
...
# Only run Sinatra if not on Vercel
unless ENV['VERCEL']
